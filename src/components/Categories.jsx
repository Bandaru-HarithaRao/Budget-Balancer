...
const Categories = () => {
  ...
  const toggleCategoryExpansion = (categoryName) => {
    ...
    // Fetch expenses for this category if not already loaded and we're expanding
    if (!expandedCategories[categoryName] && !categoryExpenses[categoryName]) {
      fetchCategoryExpenses(categoryName);
    }
  };
  ...
  const handleDeleteCategory = async (categoryName) => {
    ...
    try {
      ...
      await fetchCategories(); // Refresh categories
      // Remove from expanded categories and expenses cache
      setExpandedCategories(prev => {
        const updated = { ...prev };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setError('');
      await fetchCategories(); // Refresh categories
      // Clear the expenses cache for this category to force refresh
      setCategoryExpenses(prev => {
        const updated = { ...prev };
          ...
          {filteredCategories.length === 0 ? (
            ...
            filteredCategories.map((cat, index) => (
              <div key={index} className="category-item">
                <div className="category-header" onClick={() => toggleCategoryExpansion(cat.name)}>
                  ...
                  <div className="action-buttons">
                    <button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleEditCategory(cat.name, cat.latestAmount, cat.latestDate, cat.latestExpenseId);
                      }}
                      disabled={loading}
                    >
                      Edit
                  ...
                  <div className="expand-toggle">
                    {expandedCategories[cat.name] ? <FaChevronUp /> : <FaChevronDown />}
                  </div>
                </div>
                ...
                {expandedCategories[cat.name] && (
                  <div className="expenses-list">
                    {loadingExpenses[cat.name] ? (
                      ...
                      categoryExpenses[cat.name].length > 0 ? (
                        ...
                        <div className="no-expenses">No expenses found for this category.</div>
                      )
                    ) : (
                      <div className="loading-expenses">Loading expenses...</div>
                    )}
